...
#include "ui_AdminFormDB.h"
// Стандартные инклюды Qt для работы со стандартными моделями данных
...
AdminFormDB::AdminFormDB(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::AdminFormDB)
{
    ui->setupUi(this);
    ...
    updateEngineModels();
    connect(ui->checkBox, &QCheckBox::toggled, this, &AdminFormDB::updateEngineModels);
    connect(ui->comboBox, qOverload<int>(&QComboBox::currentIndexChanged), this, &AdminFormDB::updateEngineModels);
}

// Новый облегченный метод для подсчета судов и вывода всплывающего окна
void AdminFormDB::updateShipsCount(const QString &whereClause)
{
    ...
    // Показываем красивое всплывающее окно с информацией
    QMessageBox::information(this, "Результат фильтрации",
                             QString("Найденное число судов: <b>%1</b>").arg(shipsCount));
}

void AdminFormDB::updateEngineModels()
{
    ui->comboBox_2->clear();
...
AdminFormDB::~AdminFormDB()
{
    delete ui;
}

void AdminFormDB::displayQueryInTable(const QString &queryStr)
{
...
void AdminFormDB::setupComboBox(QComboBox *comboBox, const QString &tableName, const QString &columnName)
{
    comboBox->clear();

    // Добавляем пустую строку/подсказку
    ...
    while (query.next()) {
        ...
        // Добавляем текст, а в UserRole сохраняем ID для дальнейшей работы
        comboBox->addItem(name, id);
    }
}


void AdminFormDB::on_pushButton_clicked()
{
...
void AdminFormDB::on_pushButton_2_clicked()
{
    // Кастуем текущую модель к QStandardItemModel
    ...
    // Карта (Словарь) соответствия: номер колонки в интерфейсе -> реальное имя поля в таблице "Ships" в БД
    ...
    // Открываем транзакцию (сохранение будет атомарным и быстрым)
    db.transaction();
    bool allSavedSuccessfully = true;

    // Цикл по всем строкам таблицы в интерфейсе
    for (int row = 0; row < model->rowCount(); ++row) {
        // Из скрытой 0-й колонки вытаскиваем первичный ключ ID судна
        int shipId = model->item(row, 0)->text().toInt();

        QStringList setClauses;
        QVariantMap valuesToBind;

        // Проверяем каждую редактируемую колонку в текущей строке
        ...
        while (i.hasNext()) {
            ...
            QVariant currentValue = item->text();           // Что написано сейчас
            QVariant originalValue = item->data(Qt::UserRole); // Что было до редактирования

            // Если пользователь изменил значение ячейки
            if (currentValue != originalValue) {
                QString placeholder = QString(":field_%1").arg(i.key());
                // Добавляем конструкцию вида: "VesselName" = :field_1
                setClauses << QString("\"%1\" = %2").arg(i.value(), placeholder);
                valuesToBind[placeholder] = currentValue;
            }
        }

        // Если в текущей строке были изменения — генерируем и выполняем UPDATE запрос
        if (!setClauses.isEmpty()) {
            ...
            // Безопасно биндим (привязываем) новые значения через плейсхолдеры (защита от SQL-инъекций)
            QMapIterator<QString, QVariant> b(valuesToBind);
            while (b.hasNext()) {
                b.next();
                updateQuery.bindValue(b.key(), b.value());
            }
            // Биндим ID обновляемой строки
            updateQuery.bindValue(":id", shipId);

            // Выполняем UPDATE
            if (!updateQuery.exec()) {
                break; // Прерываем цикл, если что-то пошло не так
            }
        }
    }

    // Если все строки обновились без ошибок — фиксируем транзакцию
    if (allSavedSuccessfully) {
        db.commit();
        QMessageBox::information(this, "Успех", "Все изменения успешно сохранены в базе данных!");

        // Автоматически обновляем таблицу, чтобы актуализировать кэш исходных значений (Qt::UserRole)
        on_pushButton_clicked();
    } else {
        // Если произошла ошибка — откатываем все изменения назад, БД останется нетронутой
        db.rollback();
        QMessageBox::critical(this, "Ошибка", "Не удалось сохранить данные. Операция отменена.");
    }
}

void AdminFormDB::on_pushButton_3_clicked()
{
    adminFormUser = new AdminFormUser(this);
    adminFormUser->setAttribute(Qt::WA_DeleteOnClose);
    connect(adminFormUser, &AdminFormUser::destroyed, this, [this]() {
        adminFormUser = nullptr;
    });
    adminFormUser->show();
}
